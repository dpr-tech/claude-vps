...
# Запускать под обычным пользователем (не root) ПОСЛЕ ручного входа (`claude`).
...
if [[ ! -f "$(command -v claude 2>/dev/null)" ]]; then
  echo "Не найдена команда 'claude' — похоже, 01-server-setup.sh ещё не был выполнен." >&2
  echo "Установите Node.js и Claude Code сначала (01-server-setup.sh), затем повторите." >&2
  exit 1
fi
# Вход в аккаунт Claude на этом этапе НЕ обязателен — можно авторизоваться
# позже прямо через веб-панель (Настройки → Агенты → Claude → Войти снова).
...
echo
echo "1. НЕМЕДЛЕННО зарегистрируйте владельца панели — первый вошедший"
echo "   становится единственным владельцем."
echo "2. Войдите в аккаунт Claude прямо здесь, в браузере:"
echo "   Настройки -> Агенты -> вкладка Claude -> Аккаунт -> 'Войти снова'."
echo "   Откроется встроенный веб-терминал со ссылкой авторизации — она"
echo "   будет кликабельной, копировать вручную не нужно."
echo "============================================================"
