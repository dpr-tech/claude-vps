...
echo "  2) перенос SSH-ключа и          5) базовый firewall (ufw)"
echo "     копирование репозитория      6) Node.js + Claude Code"
echo "  3) SSH-хардненинг"
...
echo "=== Шаг 2b/6: копирование репозитория в домашнюю папку $NEW_USER ==="
CURRENT_STEP="Шаг 2b/6: копирование репозитория"
REPO_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_NAME="$(basename "$REPO_DIR")"
DEST_DIR="/home/$NEW_USER/$REPO_NAME"
if [[ "$REPO_DIR" == "$DEST_DIR" ]]; then
  echo "Репозиторий уже находится в /home/$NEW_USER — копировать не нужно."
elif [[ -d "$DEST_DIR" ]]; then
  echo "Папка $DEST_DIR уже существует, пропускаю копирование."
else
  cp -r "$REPO_DIR" "$DEST_DIR"
  chown -R "$NEW_USER:$NEW_USER" "$DEST_DIR"
  echo "Репозиторий скопирован в $DEST_DIR — доступен пользователю $NEW_USER."
fi
...
echo "     ssh $NEW_USER@$SERVER_IP"
echo "   Репозиторий уже лежит у него в домашней папке: ~/$REPO_NAME"
echo "2. Запустите веб-панель:"
echo "     cd ~/$REPO_NAME && ./02-webpanel-setup.sh"
echo "3. Вход в аккаунт Claude делать НЕ нужно здесь через SSH — сделаете"
echo "   это позже, прямо в браузере через саму веб-панель (см. README)."
echo "============================================================"
